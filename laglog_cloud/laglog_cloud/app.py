...
def load_db():
    if not os.path.exists(DATA_FILE):
        return {'orders':[], 'invoices':[], 'customers':[], 'order_counter':1}
...
def load_config():
    ...
    with open(CONFIG_FILE, encoding='utf-8') as f:
        saved = json.load(f)
    defaults.update(saved); return defaults
...
LABELS = {
    'de': {
        ...
        'reminder_text':'wir erlauben uns, Sie freundlich an die Begleichung der nachfolgenden Rechnung zu erinnern.',
        'dunning_text':'trotz unserer Zahlungserinnerung ist die nachfolgende Rechnung noch nicht beglichen. Wir fordern Sie auf, den ausstehenden Betrag umgehend zu überweisen.',
    ...
    'en': {
        ...
        'reminder_text':'we kindly remind you to settle the following invoice.',
        'dunning_text':'despite our payment reminder, the following invoice remains unpaid. We urge you to transfer the outstanding amount immediately.',
