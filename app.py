...
# Set JSONBIN_KEY and JSONBIN_BIN_ID as environment variables on Render
JSONBIN_KEY    = os.environ.get('JSONBIN_KEY', '')
JSONBIN_BIN_DB = os.environ.get('JSONBIN_BIN_DB', '')   # bin for orders/invoices/customers
JSONBIN_BIN_CF = os.environ.get('JSONBIN_BIN_CF', '')   # bin for config
...
def _jb_get(bin_id):
    """Read from JSONBin — returns parsed dict or None on error"""
...
def _jb_put(bin_id, data):
    """Write to JSONBin — returns True on success"""
    if not JSONBIN_KEY or not bin_id:
        return False
    try:
...
# ── DB helpers ────────────────────────────────────────────
_db_cache = None  # in-memory cache to reduce JSONBin API calls

def load_db():
    global _db_cache
    empty = {'orders':[], 'invoices':[], 'customers':[], 'order_counter':1}
    # 1. Try JSONBin
    if JSONBIN_KEY and JSONBIN_BIN_DB:
        data = _jb_get(JSONBIN_BIN_DB)
        if data and isinstance(data, dict):
            for k,v in [('customers',[]),('order_counter',len(data.get('orders',[]))+1)]:
...
def save_db(db):
    ...
    # 1. Save to JSONBin
    if JSONBIN_KEY and JSONBIN_BIN_DB:
        _jb_put(JSONBIN_BIN_DB, db)
    # 2. Always also save locally as backup
    try:
        with open(DATA_FILE, 'w', encoding='utf-8') as f:
...
def load_config():
    ...
    # Try JSONBin
    if JSONBIN_KEY and JSONBIN_BIN_CF:
        data = _jb_get(JSONBIN_BIN_CF)
        if data and isinstance(data, dict):
...
def next_order_nr():
    ...
    return jsonify({'nr': f'LL-TA-{year}-{str(counter).zfill(4)}'})
...
def ping():
    ...
    return jsonify({
        'ok': True,
        'storage': 'jsonbin' if (JSONBIN_KEY and JSONBIN_BIN_DB) else 'local_file',
        'jsonbin_configured': bool(JSONBIN_KEY and JSONBIN_BIN_DB),
...
def _smtp_connect(cfg):
    ...
    if not host: raise ValueError('SMTP-Host nicht konfiguriert.')
    port = int(cfg.get('smtp_port',587))
    if cfg.get('smtp_ssl'):
        srv = smtplib.SMTP_SSL(host, port, timeout=10)
    else:
        srv = smtplib.SMTP(host, port, timeout=10); srv.ehlo(); srv.starttls(); srv.ehlo()
    u,p = cfg.get('smtp_user',''), cfg.get('smtp_pass','')
    if u and p: srv.login(u, p)
    return srv
